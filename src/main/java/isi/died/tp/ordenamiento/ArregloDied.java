...
public class ArregloDied {
    ...
    //Metodo para obtener el mayor elemento del arreglo:
    public long getMax()
    {
        long mx = this.datos[0].valorOrdenamiento();
        for (int i = 1; i < this.datos.length; i++){
            ArregloDied.comparaciones++;
            if (this.datos[i].valorOrdenamiento() > mx)
                mx = this.datos[i].valorOrdenamiento();
        }
        return mx;
    }

    public Long getValorOrdenable(int pos) {
        return this.datos[pos].valorOrdenamiento();
    }
